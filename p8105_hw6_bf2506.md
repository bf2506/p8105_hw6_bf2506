...
as predictors). It has a low RMSE. I think it is because I did not use
data that can only be measured at birth (e.g., bhead, blength). My
hypothetical structural thinking considers a regression model when
infant birth data are not available. That is why my RMSE is bigger.
